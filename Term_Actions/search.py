...
class FileSearcher:
    ...
    def trim(self, item_list: list, mode: str = "folders") -> list:
        ...
        if mode != "folders":
            temp_files = [
                ...
                if "." in file.name and not file.name.startswith(".")
            ]
            return temp_files
        self.ignore_folders = [
            "Public",
        ...
        temp_folds = [
            folder.name
            for folder in item_list
            if "." not in folder.name and folder.name not in self.ignore_folders
        ]
        return temp_folds
    ...
    def check_parents(self, folders: list) -> None:
        ...
        self.folders = list(
            map(
                lambda x: self.DataMaker.make_folder_path(
                    f"{settings.ROOT_DIR}/{x}", file_path=self.inputs[3]
                ),
                folders,
            )
        )
                ...
                for parent_dir in self.folders:
                    ...
                    for sub_directory in subdirectories:
                        ...
                        if self.check_with_result(self.inputs[0], sterilized_search):
                            each_folder_changed = self.change_for_results(
                                parent_dir, sub_directory
                            )
                            self.results.append(each_folder_changed)
                        self.sub_folders.append(
                            self.DataMaker.make_folder_path(
                                f"{parent_dir.path}/{sub_directory}",
                                file_path=self.inputs[3],
                            )
                        )
                self.folders = self.sub_folders  # Setting up for next iteration
                self.sub_folders = (
                    []
                )  # Resetting the Sub folders list for easy transition
...
if __name__ == "__main__":
    ...
    if target == "" or target == "-a":
        ...
        mode = ""
    if mode == "":
        search_instance = FileSearcher(
            folder, target, record_time=True, absolute=absolute_search
        )
        ...
        search_instance = FileSearcher(
            folder, mode=mode, record_time=True, absolute=absolute_search
        )
