...
class RepoMerger:
    ...
    def merge(self, title, body, all=False):
        if all == False:
            self.pull_details = self.create_pull_request(title=title, body=body)
            self.merge_branches(
            ...
            for base, heads in final_execution_list.items():
                ...
                for head in heads:
                    try:
                        pull_details = self.create_pull_request(
                            title=f"Merge to {base}", body=body, base=base, head=head
                        )
