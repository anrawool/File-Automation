...
class DataMaker:
    ...
    def make_folder_path(
        self, path, folder="auto", ext="auto", file="auto", file_path=True
    ):
        if file == "auto":
            if "." in path:
        ...
        if folder == "auto":
            ...
            folder = folder
        self.fold_path = path
        fold_object = NexusFolderPathObject(
            path=self.fold_path, folder=folder, file=file, ext=ext
        )
        return fold_object
    ...
    def change_path(self, object, **change_vals):
        ...
        if "path" in change_vals:
            ...
            object = self.make_path(
                path=key_val["path"], file=key_val["file"], ext=key["ext"]
            )
        return object
