...
class SSH_Connection:
    def __init__(self, username, ip='auto', password='auto') -> None:
        self.manager =  PasswordManager(db_path = './Managers/Password_Manager/passwords.sqlite', key_path='./Managers/Password_Manager/passwords_key.json')
        self.AEA = AEA(key_path='./Managers/Password_Manager/passwords_key.json')
        if ip == 'auto':
            ip = '192.168.1.58'
        if password == 'auto':
