...
class Gmail_Connection:
    def __init__(
        ...
        if gmail_address != None and gmail_password != None:
            ...
            self.gmail_connection.login(info.Email_Address, "cymu srqt fhnn nsoe")
        else:
    ...
    def convert_date(self, date_time_string):
        parsed_date_time = datetime.strptime(
            date_time_string, "%a, %d %b %Y %H:%M:%S %z"
        )
            ...
            for response in msg:
                if isinstance(response, tuple):
                    ...
                    for part in msg.walk():
                        ...
                        if body.strip():
                            email_object = NexusEmailObject(
                                sender_address=from_,
                                receiver_address=info.Email_Address,
                                date=date_str,
                                subject=subject,
                                body=body,
...
email_connection = Gmail_Connection(criteria='FROM "sarthkrawool09@gmail.com"')
