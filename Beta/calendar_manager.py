...
class CalendarInstance:
    ...
    def create_event(self, options : dict, get_link=False):
        try:
            ...
            event = {
                ...
                "attendees": [
                    {"email": "sarthakrawool09@gmail.com"}
                ]
            }
    ...
    def read_calendar(self, calendar_id='primary', max_results=10, single_events=True, order_by='startTime'):
        calendar_id = self.find_calendar_id(calendar_name=calendar_id)
        try:
            service = build("calendar", "v3", credentials=self.creds)
            now = dt.datetime.now().isoformat() + "Z"
            event_result = service.events().list(calendarId=calendar_id, timeMin=now, maxResults=max_results, singleEvents=single_events, orderBy=order_by).execute()
