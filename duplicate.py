    ...
    def name(self, folder_loc):
        ...
        for i in folder_files:
            if self.target in i.name.lower():
                ...
                if ext == '':
                    folder_append = self.DataMakerObj.make_folder_path(path=set_path, file_path=self.file_path)
                else:
    ...
    def ext(self, folder_loc):
        ...
        for i in folder_files:
            if i.name.endswith(self.target):
                ...
                set_path = f'{folder_loc}/{self.target_folder}/{i.name}'
                set_path = re.sub('/+', '/', set_path)
                file_append = self.DataMakerObj.make_folder_path(path=set_path, file=file, ext=ext, file_path=self.file_path)
                ...
                for each_folder in self.folders:
                    ...
                    for dir in dirs:
                        ...
                        if self.target_folder == search_dir:
                            ...
                            each_folder.path = each_folder.path.replace(f'/{self.target_folder}', '')
                            each_folder.folder = each_folder.path.replace(os.path.dirname(each_folder.path))
